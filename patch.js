/* 2026-09-17 UI patch: rename 自由 -> 彈性, restaurant sub-categories, Day 1 dinner details, verified Maedaya Hakata data */
(function(){
  const originalRender=window.render;
  state.foodCat=state.foodCat||'全部';
  const day1Restaurants=[
    {region:'福岡酒店附近',cat:'咸食',name:'博多もつ鍋 前田屋 博多店',jp:'博多もつ鍋前田屋 博多店',desc:'牛雜鍋｜博多代表性鍋物，使用國產和牛小腸；可選味噌、醬油或辣味鍋。',address:'福岡市博多区博多駅前3-26-5 Mビル1F',hours:'11:00–14:30（LO 14:00）；17:00–24:00（食事LO 23:00／飲物LO 23:30）',phone:'092-482-8558',img:'https://cdn-ak.f.st-hatena.com/images/fotolife/v/v2133v/20191017/20191017212647.jpg'},
  ...
  function card(r){
    const mapUrl=maps(r.name,r.address);
    return `<article class="restaurant"><img class="restaurant-img" src="${r.img}" alt="${safe(r.name)} 食物照片" loading="lazy"><div class="restaurant-body"><div class="restaurant-title"><div><h3>${safe(r.name)}</h3><div class="jp">${safe(r.jp)}</div></div><span class="category">${safe(r.cat)}</span></div><p class="desc">${safe(r.desc)}</p><div class="address">📍 ${safe(r.address)}</div>${r.hours?`<div class="hours">🕐 ${safe(r.hours)}</div>`:''}${r.phone?`<div class="hours">☎️ ${safe(r.phone)}</div>`:''}<div class="restaurant-actions"><a class="map-btn" href="${mapUrl}" target="_blank" rel="noopener noreferrer">📍 Google Maps 導航</a><a class="web-btn" href="https://www.google.com/search?q=${encodeURIComponent(r.name+' 福岡') }" target="_blank" rel="noopener noreferrer">搜尋店家</a></div><div class="map-url-label">Google Maps：點擊「Google Maps 導航」即可開啟地圖定位</div></div></article>`
  }
