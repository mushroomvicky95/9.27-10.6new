/* Day 1 dinner layout + no-duplicate restaurant rules */
(function(){
'use strict';
...
const itineraryRestaurants=[
  '博多もつ鍋 前田屋','前田屋','水たき 長野','NOOICE tenjin','いくら博多店','manu coffee 大名店',
  'やりうどん福岡店','みやま本舗','焼肉なべしま','櫻島市場食堂','お食事処海月',
  '鹿兒島屋台村','めっけもん','新港食堂','大衆酒場かどや','かどや','とりくら','豚とろ',
  'danken COFFEE','可否三昧','Voila Coffee','くじらcafé','うなぎの末よし','Café Cochi',
  '天文館むじゃき本店','ざぼんラーメン','いちにいさん','Gyudo!'
];
function norm(s){return String(s||'').replace(/\s+/g,'').toLowerCase()}
const duplicateKeys=itineraryRestaurants.map(norm);
function isFlexPage(){
  const active=[...document.querySelectorAll('.tab.active span')].some(s=>s.textContent.trim()==='彈性');
  return active || !!document.querySelector('.filter-row .filter');
}
function removeItineraryDuplicates(){
  if(!isFlexPage())return;
  document.querySelectorAll('.restaurant').forEach(card=>{
    if(card.classList.contains('day1-card'))return;
    const text=norm(card.textContent);
    if(duplicateKeys.some(k=>k && text.includes(k)))card.classList.add('flex-duplicate-hidden');
  });
  document.querySelectorAll('.region-block').forEach(region=>{
    const visible=[...region.querySelectorAll('.restaurant')].some(x=>!x.classList.contains('flex-duplicate-hidden'));
    if(region.querySelector('.restaurant') && !visible)region.classList.add('flex-duplicate-hidden');
  });
}
...
function placeFixed(event){
  ...
  if(box && box.parentElement!==event){box.remove();box=null}
  if(!box){
    event.insertAdjacentHTML('beforeend',dinner);
    box=event.querySelector('.day1-dinner-fixed');
  }
  if(box){box.style.gridColumn='2';box.style.width='100%';box.style.minWidth='0'}
}
function inject(){
  rename();
  const event=findDinnerEvent();
  if(event)placeFixed(event);
  removeItineraryDuplicates();
}
