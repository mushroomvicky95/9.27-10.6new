...
(function(){
  ...
  function catOf(r){
    ...
    if(/酒|sake|craft/.test(c)) return '酒類';
    if(/咖啡|甜品|可麗|水果派|咖啡豆/.test(c)) return '咖啡甜品';
    return '咸食';
  }
  ...
  function tabsRename(){
    document.querySelectorAll('.tab').forEach(b=>{const s=b.querySelector('span');if(s&&s.textContent.trim()==='自由')s.textContent='彈性';});
  }
  function freeView(){
    const q=(state.query||'').trim().toLowerCase();
    const region=state.region||'全部';
    ...
    const rr=state.foodCat==='全部'?all:all.filter(r=>catOf(r)===state.foodCat);
    const cats=['全部','咸食','咖啡甜品','酒類'];
  ...
  function addDay1Dinner(){
    ...
    const target=events.find(e=>e.querySelector('h3')&&e.querySelector('h3').textContent.includes('天神晚餐選擇'));
    if(!target)return;
    const holder=document.createElement('div');
    holder.className='day1-dinner';
    holder.innerHTML=`<div class="day1-label">🍲 晚餐餐廳資訊</div>${day1Restaurants.map(card).join('')}`;
    target.querySelector('.event-card').appendChild(holder);
  }
  function renderPatched(){
    if(state.tab==='free'){
      originalRender();
      document.querySelector('.content').innerHTML=freeView();
    }else{
      originalRender();
      addDay1Dinner();
    }
